...
final class ContentViewModel: NSObject, ObservableObject, CLLocationManagerDelegate {
    ...
    private func checkLocationAuthorization() {
        ...
        switch locationManager.authorizationStatus{
            ...
            case .authorizedAlways, .authorizedWhenInUse:
                region = MKCoordinateRegion(center: locationManager.location?.coordinate ?? MapDetails.startingLocation, span: MKCoordinateSpan(latitudeDelta: 0.05, longitudeDelta: 0.05))
            @unknown default:
                break
        }
    }
    ...
    func retrieveUserLocation() -> CLLocationCoordinate2D {
        if CLLocationManager.locationServicesEnabled() {
            return locationManager?.location?.coordinate ?? CLLocationCoordinate2D(latitude: 1.3414, longitude: 103.9633)
        } else {
            print("show an alert to ask them to turn on location service")
